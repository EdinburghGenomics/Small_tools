...
def filter_duplicated_unmpped_read(input_file_path, output_file_path):
    ...
    sys.stderr.write('Wrote %s reads to %s\n' % (total_reads - reads_skipped, output_file_path))
...
def _prepare_argparser():
    """Prepare optparser object. New arguments will be added in this function first."""
    argparser = ArgumentParser()
    argparser.add_argument("-i", "--input_bam_file", type=str, required=True,
                           help="The bam file containing the reads to filter.")
    argparser.add_argument("-o", "--output_bam_file", type=str, required=True,
